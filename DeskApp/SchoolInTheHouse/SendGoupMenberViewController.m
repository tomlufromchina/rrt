//
//  SendGoupMenberViewController.m
//  RenrenTong
//
...
#import "NextSendGroupMenberCell.h"
#import "NetWorkManager+SchoolAndHouse.h"
#import "XYAlertViewHeader.h"

@interface SendGoupMenberViewController ()<UITableViewDataSource,UITableViewDelegate,UITextViewDelegate>
{
    NSString *numberStr;
...
@implementation SendGoupMenberViewController
...
- (void)requestData
{
    ...
    SendGroupMenberCell *cell1 = (SendGroupMenberCell *)[self.mainTableView cellForRowAtIndexPath:cellPath1];
    cell1 = (SendGroupMenberCell *)[self.mainTableView cellForRowAtIndexPath:cellPath1];
    
    
    NSIndexPath *cellPath = [NSIndexPath indexPathForItem:0 inSection:0];
    NextSendGroupMenberCell *cell = (NextSendGroupMenberCell*)[self.mainTableView cellForRowAtIndexPath:cellPath];
    
    cell = (NextSendGroupMenberCell*)[self.mainTableView cellForRowAtIndexPath:cellPath];
    // 以短信形式发送
    if (cell1.messageButton.tag == 1) {
        ...
        for (int i = 0; i < [self.groupMenberInfo count]; i ++) {
            NSString *objectIdList = [NSString stringWithFormat:@"objectIdList=%@",self.groupMenberInfo[i]];
            
            [self.netWorkManager teacherSendNextMessageForSeveralMenber:[RRTManager manager].loginManager.loginInfo.tokenId
    ...
    // 以网页形式发送
        ...
        for (int i = 0; i < [self.groupMenberInfo count]; i ++) {
            NSString *objectIdList = [NSString stringWithFormat:@"objectIdList=%@",self.groupMenberInfo[i]];
            [self show];
            [self.netWorkManager teacherSendMessageForSeveralMenver:[RRTManager manager].loginManager.loginInfo.tokenId
...
{
    if (indexPath.section == 0) {
        static NSString *cellIdentifier = @"NextSendGroupMenberCell";
        NextSendGroupMenberCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"NextSendGroupMenberCell" owner:self options:nil] lastObject];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        return cell;
        
    } else {
        static NSString *cellIdentifier = @"SendGroupMenberCell";
        SendGroupMenberCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"SendGroupMenberCell" owner:self options:nil] lastObject];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        return cell;
    }
