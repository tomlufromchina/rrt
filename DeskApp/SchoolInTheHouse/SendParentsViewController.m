...
#import "SendParentsCell.h"
#import "NextSendParentsCell.h"
#import "NetWorkManager+SchoolAndHouse.h"
#import "XYAlertViewHeader.h"

@interface SendParentsViewController ()<UITableViewDataSource,UITableViewDelegate,UITextViewDelegate>
{
    NSString *numberStr;
...
@implementation SendParentsViewController
...
- (void)requestData
{
    ...
    SendParentsCell *cell1 = (SendParentsCell *)[self.mainTableView cellForRowAtIndexPath:cellPath1];
    cell1 = (SendParentsCell *)[self.mainTableView cellForRowAtIndexPath:cellPath1];
    
    
    NSIndexPath *cellPath = [NSIndexPath indexPathForItem:0 inSection:0];
    NextSendParentsCell *cell = (NextSendParentsCell*)[self.mainTableView cellForRowAtIndexPath:cellPath];
    
    cell = (NextSendParentsCell*)[self.mainTableView cellForRowAtIndexPath:cellPath];
    ...
    if (cell1.messageButton.tag == 1) {
        ...
        for (int i = 0; i < [self.studentId count]; i ++) {
            NSString *objectIdList = [NSString stringWithFormat:@"objectIdList=%@",self.studentId[i]];
            [self show];
            [self.netWorkManager teacherSendNextMessageForSeveralParents:[RRTManager manager].loginManager.loginInfo.tokenId
                                                             messageType:messageType
                                                                 message:self.textView.text classId:self.classId
    ...
    // 以网页形式发送
        ...
        for (int i = 0; i < [self.studentId count]; i ++) {
            NSString *objectIdList = [NSString stringWithFormat:@"objectIdList=%@",self.studentId[i]];
            [self show];
            [self.netWorkManager teacherSendMessageForSeveralParents:[RRTManager manager].loginManager.loginInfo.tokenId
                                                           teacherId:[RRTManager manager].loginManager.loginInfo.userId
                                                         messageType:messageType
                                                             message:self.textView.text
                                                        objectIdList:objectIdList success:^(NSMutableArray *data) {
                                                            [self dismiss];
                                                            [self gotoUpdataUI:data];
...
{
    if (indexPath.section == 0) {
        static NSString *cellIdentifier = @"NextSendParentsCell";
        NextSendParentsCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"NextSendParentsCell" owner:self options:nil] lastObject];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        return cell;
        
    } else {
        static NSString *cellIdentifier = @"SendParentsCell";
        SendParentsCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"SendParentsCell" owner:self options:nil] lastObject];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        return cell;
    }
