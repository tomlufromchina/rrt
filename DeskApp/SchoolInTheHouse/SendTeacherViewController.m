...
#import "SendTeacherCell.h"
#import "NextSendTeacherCell.h"
#import "NetWorkManager+SchoolAndHouse.h"
#import "XYAlertViewHeader.h"

@interface SendTeacherViewController ()<UITableViewDataSource,UITableViewDelegate,UITextViewDelegate>
{
    NSString *numberStr;
...
@implementation SendTeacherViewController
...
- (void)requestData
{
    ...
    SendTeacherCell *cell1 = (SendTeacherCell *)[self.mainTableView cellForRowAtIndexPath:cellPath1];
    cell1 = (SendTeacherCell *)[self.mainTableView cellForRowAtIndexPath:cellPath1];
    
    
    NSIndexPath *cellPath = [NSIndexPath indexPathForItem:0 inSection:0];
    NextSendTeacherCell *cell = (NextSendTeacherCell*)[self.mainTableView cellForRowAtIndexPath:cellPath];
    
    cell = (NextSendTeacherCell*)[self.mainTableView cellForRowAtIndexPath:cellPath];
    // 以短信形式发送
    if (cell1.messageButton.tag == 1) {
        int messageType = 0;
        if (cell.noticeButton.tag == 1) {
            messageType = 11;
        } else if (cell.taskButton.tag == 1){
            messageType = 8;
        } else if (cell.performanceButton.tag == 1){
            messageType = 10;
        } else if (cell.wishButton.tag == 1){
            messageType = 9;
        }
        
        for (int i = 0; i < [self.teacherIdArray count]; i ++) {
            NSString *objectIdList = [NSString stringWithFormat:@"objectIdList=%@",self.teacherIdArray[i]];
            [self.netWorkManager bossSendNextMessageForTeachers:[RRTManager manager].loginManager.loginInfo.tokenId
    ...
    // 网页形式发送
        ...
        for (int i = 0; i < [self.teacherIdArray count]; i ++) {
            ...
            [self.netWorkManager bossSendMessageForTeachers:[RRTManager manager].loginManager.loginInfo.tokenId
                                              SenderAccount:[RRTManager manager].loginManager.loginInfo.userId
                                                messageType:messageType
                                                    Message:self.textView.text
                                               ObjectIdList:objectIdList
                                                    success:^(NSMutableArray *data) {
...
{
    if (indexPath.section == 0) {
        static NSString *cellIdentifier = @"NextSendTeacherCell";
        NextSendTeacherCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"NextSendTeacherCell" owner:self options:nil] lastObject];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        return cell;
        
    } else {
        static NSString *cellIdentifier = @"SendTeacherCell";
        SendTeacherCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"SendTeacherCell" owner:self options:nil] lastObject];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        return cell;
    }
...
- (UIView*)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    if (section == 1) {
        ...
        self.placeholderLabel.text = @"输入你要发的文字...";
