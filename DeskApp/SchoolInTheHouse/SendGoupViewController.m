...
#import "SendGroupCell.h"
#import "NextSendGroupCell.h"
#import "NetWorkManager+SchoolAndHouse.h"
#import "XYAlertViewHeader.h"

@interface SendGoupViewController ()<UITableViewDataSource,UITableViewDelegate,UITextViewDelegate>
{
    NSString *numberStr;
...
@implementation SendGoupViewController
...
- (void)requestData
{
    ...
    SendGroupCell *cell1 = (SendGroupCell *)[self.mainTableView cellForRowAtIndexPath:cellPath1];
    cell1 = (SendGroupCell *)[self.mainTableView cellForRowAtIndexPath:cellPath1];
    
    
    NSIndexPath *cellPath = [NSIndexPath indexPathForItem:0 inSection:0];
    NextSendGroupCell *cell = (NextSendGroupCell*)[self.mainTableView cellForRowAtIndexPath:cellPath];
    
    cell = (NextSendGroupCell*)[self.mainTableView cellForRowAtIndexPath:cellPath];
    ...
    if (cell1.messageButton.tag == 1) {
        ...
        for (int i = 0; i < [self.classIdInfo count]; i ++) {
            NSString *objectIdList1 = [NSString stringWithFormat:@"groupId=%@",self.classIdInfo[i]];
            [self show];
            [self.netWorkManager teacherSendNextMessageForGroup:[RRTManager manager].loginManager.loginInfo.tokenId
                                                      teacherId:[RRTManager manager].loginManager.loginInfo.userId
                                                    messageType:messageType
                                                        message:self.textView.text
                                                   objectIdList:objectIdList1
    ...
    // 以网页形式发送
        ...
        for (int i = 0; i < [self.classIdInfo count]; i ++) {
            ...
            [self show];
            [self.netWorkManager teacherSendMessageForGroup:[RRTManager manager].loginManager.loginInfo.tokenId
                                                  teacherId:[RRTManager manager].loginManager.loginInfo.userId
                                                messageType:messageType
                                                    message:self.textView.text
...
{
    if (indexPath.section == 0) {
        static NSString *cellIdentifier = @"NextSendGroupCell";
        NextSendGroupCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"NextSendGroupCell" owner:self options:nil] lastObject];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        return cell;
        
    } else {
        static NSString *cellIdentifier = @"SendGroupCell";
        SendGroupCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"SendGroupCell" owner:self options:nil] lastObject];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        return cell;
    }
