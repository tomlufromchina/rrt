...
{
    NSString *resultStr = title ? title : defaultStr;
    NSTimeInterval displayInterval = [[SVProgressHUD sharedView] displayDurationForString:resultStr];
    [[self sharedView] showImage:nil status:resultStr duration:displayInterval];
    
}

+ (void)showWithTitle:(NSString *)title withTime:(NSTimeInterval)time
{
    [[self sharedView] showImage:nil status:title duration:time];
}
...
- (void)updatePosition {
    ...
    if(string) {
        CGSize stringSize = [string calculateSize:CGSizeMake(200, 300) font:self.stringLabel.font];
