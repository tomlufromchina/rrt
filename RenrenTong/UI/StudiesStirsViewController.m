...
@interface StudiesStirsViewController ()<ModificationVCDelegate>
{
...
@implementation StudiesStirsViewController
...
#warning 期末通知书 明年重做 目前自定义推送 不支持
    ...
    if (SCREENHEIGHT == 480) {
        ...
        self.endView.left = 0;
        self.endView.top = self.addressLabel.bottom + 5;
        self.endView.width = SCREENWIDTH;
        self.endView.bottom = self.headerView.bottom + 10;
    ...
    self.ZYButton.top = self.headerView.bottom - 10;
    self.CJButton.top = self.headerView.bottom - 10;

    self.ZYButton.left = (SCREENWIDTH/2 - 100)/2 + 30;
    self.CJButton.left = (SCREENWIDTH/2 - 100)/2 + SCREENWIDTH/2 - 10;
...
//    self.headerView.backgroundColor = homeHeaderColor;
    ...
    self.ZYLabel.left = (SCREENWIDTH/2 - 100)/2 + 30;
    self.CJLabel.left = (SCREENWIDTH/2 - 100)/2 + SCREENWIDTH/2 - 10;
...
#pragma mark -- iPhone4适配

- (void)iPhone4UIAdaptive
{
...
- (void)BushBaoCun
{
    ...
    MessageCountArray = [userDefaults1 arrayForKey:@"TheMessageCount2"];
...
- (void)noticeTapGesture:(UIGestureRecognizer *)ges
{
    ...
    NSString *theEndURL = [userDefaults stringForKey:@"TheURL2"];
    ...
    [defaults removeObjectForKey:@"TheMessageCount2"];
...
- (void)requestUIData
{
    __weak StudiesStirsViewController *_self = self;
...
- (void)gotoTheUI:(NSMutableArray *)data
{
    if (data && [data count] > 0) {
        self.userName.text = [NSString stringWithFormat:@"%@ (学生)",[RRTManager manager].loginManager.loginInfo.userName];
        ...
        BOOL issettingid=[nud boolForKey:[NSString stringWithFormat:@"%@isSettingId1",[RRTManager manager].loginManager.loginInfo.userId]];
        if (issettingid) {
            [self issettinged];
        }else{
...
//                                           TheStudentsViewController *VC = (TheStudentsViewController *)viewController;
//                                           VC.theTitle = @"作业";
//                                           //                                       VC.messageType = 8;
//                                       }];
//    }
    if (tmpNumber==-1||tmpNumber==1) {
        ...
        [self.navigationController pushViewController:TheNoticeVCID
                                       withStoryBoard:AboutMeStoryBoardName withBlock:^(UIViewController *viewController) {
                                           TheNoticeViewController *VC = (TheNoticeViewController *)viewController;
                                           VC.theTitle = @"作业";
                                           VC.messageType = 8;
                                       }];
    }
