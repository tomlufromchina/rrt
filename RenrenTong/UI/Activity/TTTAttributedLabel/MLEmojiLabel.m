...
//REGULAREXPRESSION(PhoneNumerRegularExpression, @"1+[3458]+\\d{9}|(\\d{4}|\\d{3})-(\\d{7,8})")  //电话
...
REGULAREXPRESSION(UserRegularExpression, @"U(\\[(.*?)+\\])U.[0-9]{1,30}+I([0-9]{1,500})IC([0-9]{1,500})C ")//用户名
...
NSString * const kURLActions[] = {@"user->",@"url->",@"phoneNumber->",@"email->",@"at->",@"poundSign->"};
...
+ (NSDictionary *)emojiDictionary {
    ...
    static dispatch_once_t onceToken;
	dispatch_once(&onceToken, ^{
        NSString *plistPath = [[NSBundle mainBundle] pathForResource:@"faceMap_ch" ofType:@"plist"];
        NSDictionary *dictionary = [[NSDictionary alloc] initWithContentsOfFile:plistPath];
        NSMutableDictionary* temp=[[NSMutableDictionary alloc] init];
        for (int i=0; i<[dictionary count]; i++) {
            [temp setObject:[NSString stringWithFormat:@"f%03d.png", i] forKey:[dictionary objectForKey:[NSString stringWithFormat:@"f%03d", i]]];
        }
	    emojiDictionary = temp;
	});
...
//    [self initPlist];
    ...
    NSMutableDictionary *usermutableActiveLinkAttributes = [NSMutableDictionary dictionary];
    [usermutableActiveLinkAttributes setObject:[NSNumber numberWithBool:NO] forKey:(NSString *)kCTUnderlineStyleAttributeName];
    ...
    UIColor *commonLinkColor = [UIColor colorWithRed:0.112 green:0.000 blue:0.791 alpha:1.000];//蓝色
    
    UIColor *userLinkColor = [UIColor colorWithRed:0.043 green:0.650  blue:0.372  alpha:1.000];//绿色
    if (self.UserTextColor) {
        userLinkColor=self.UserTextColor;
    }
    //点击时候的背景色
    ...
    if ([NSMutableParagraphStyle class]) {
        ...
        [usermutableActiveLinkAttributes setObject:userLinkColor forKey:(NSString *)kCTForegroundColorAttributeName];
        ...
        [usermutableActiveLinkAttributes setObject:(__bridge id)[userLinkColor CGColor] forKey:(NSString *)kCTForegroundColorAttributeName];
    ...
    self.userLinkAttributes=[NSDictionary dictionaryWithDictionary:usermutableActiveLinkAttributes];
    
    [usermutableActiveLinkAttributes setValue:(__bridge id)[[UIColor colorWithWhite:0.631 alpha:1.000] CGColor] forKey:(NSString *)kTTTBackgroundFillColorAttributeName];
    self.useractiveLinkAttributes=[NSDictionary dictionaryWithDictionary:usermutableActiveLinkAttributes];
...
+(int)getUserid:(NSString*)regstr{
    if ([regstr length]>0) {
        NSArray  * array= [regstr componentsSeparatedByString:@"]U"];
        if ([array count]==2) {
...
+(int)getIndex:(NSString*)regstr{
    if ([regstr length]>0) {
        NSArray  * array= [regstr componentsSeparatedByString:@"I"];
        if ([array count]==3) {
            return [array[1] intValue];
        }else{
            return 0;
...
+(int)getCommentID:(NSString*)regstr{
    if ([regstr length]>0) {
        NSArray  * array= [regstr componentsSeparatedByString:@"C"];        
        if ([array count]==3) {
            return [array[1] intValue];
        }else{
            return 0;
...
- (void)setEmojiText:(NSString*)emojiText
{
    ...
    NSRegularExpression * const regexps[] = {kUserRegularExpression(),kURLRegularExpression(),kEmailRegularExpression(),kAtRegularExpression(),kPoundSignRegularExpression()};
    ...
    for (NSUInteger i=0; i<maxIndex; i++) {
        ...
        [regexps[i] enumerateMatchesInString:[mutableAttributedString string] options:0 range:stringRange usingBlock:^(NSTextCheckingResult *result, __unused NSMatchingFlags flags, __unused BOOL *stop) {
            ...
            [tempresults addObject:aResult];
        }];
    }
    NSArray *sortedArray = [tempresults sortedArrayUsingComparator:^NSComparisonResult(NSTextCheckingResult *p1, NSTextCheckingResult *p2){
        return [[NSNumber numberWithInt:p1.range.location] compare:[NSNumber numberWithInt:p2.range.location]];
    }];
    NSInteger indent=0;
    NSMutableAttributedString* tempattrstr=[[NSMutableAttributedString  alloc] initWithAttributedString:self.attributedText];
    for (NSTextCheckingResult *tempresult in sortedArray) {
        NSArray  * nameandid= [tempresult.replacementString componentsSeparatedByString:@"->"];
        if ([nameandid[0] isEqualToString:@"user"]) {
            NSArray  * array= [nameandid[1] componentsSeparatedByString:@"]U"];
            NSString* name=[array[0] stringByReplacingOccurrencesOfString:@"U[" withString:@""];
            NSInteger location=0;
            if (tempresult.range.location!=0) {
                location=tempresult.range.location-indent;
            }
            indent+=tempresult.range.length-[name length];
            NSRange range = NSMakeRange(location,tempresult.range.length);
            [tempattrstr replaceCharactersInRange:range withString:name];
            ...
            [self.linkvalues addObject:name];
        }else{
            NSRange range = NSMakeRange(tempresult.range.location-indent,tempresult.range.length);
            NSTextCheckingResult* result=[NSTextCheckingResult spellCheckingResultWithRange:range];
            [results addObject:result];
            [self.linkvalues addObject:tempresult.replacementString];
        }
    }
    [self setText:tempattrstr];
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
    //这里直接调用父类私有方法，好处能内部只会setNeedDisplay一次。一次更新所有添加的链接
    NSMutableArray *mutableLinks = [[NSMutableArray alloc] init];
    for (NSTextCheckingResult *result in sortedArray) {
        [mutableLinks addObject:result];
    }
    self.infolinks = mutableLinks;
