//
//  MyselfTendencyViewController.m
//  RenrenTong
//
...
@interface MyselfTendencyViewController ()<UITableViewDataSource,UITableViewDelegate,DynamicCellDelegate,commentaryToolViewDelegate>
{
...
@implementation MyselfTendencyViewController
...
- (void)requestData
{
//        [SocialNetService GetMyActivity:[RRTManager manager].loginManager.loginInfo.userId
//                                 TypeId:@""
...
//                             } Cache:^(id model) {
//                                 if (model && self.pageIndex > 1) {
//                                     tempCacheAry = (NSMutableArray*)model;
//                                 }
//                                 [self updateView:model];
//                             }];
    
    NSString *url = [NSString stringWithFormat:@"http://home.%@/api/GetMyActivity",aedudomain];
    NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:@"",@"typeId",[RRTManager manager].loginManager.loginInfo.userId,@"userId",[NSString stringWithFormat:@"%d",self.pageIndex],@"pageIndex",nil];
    [HttpUtil GetWithUrl:url parameters:dic success:^(id json) {
        ...
        if (theMyTendency.st == 0) {
            if (theMyTendency && self.pageIndex > 1) {
                tempCacheAry = [theMyTendency.msg.list mutableCopy];
            }
            [self updateView:[theMyTendency.msg.list mutableCopy]];
            isHeadEndRefresh = YES;
            isFootEndRefresh = YES;
...
{
    __weak MyselfTendencyViewController *_self = self;
    [self show];
