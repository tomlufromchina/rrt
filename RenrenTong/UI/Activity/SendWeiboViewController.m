    ...
    if (self.images && [self.images count] > 0) {
        ...
        if ([self validataTheSend] && isEndPublish) {
            ...
            [self setRequest:[ASIFormDataRequest requestWithURL:
                              [NSURL URLWithString:[NSString stringWithFormat:@"http://home.%@/Api/PostUploadImages",aedudomain]]]];
        ...
        if ([self validataTheSend] && isEndPublish) {
            ...
            [self.netWorkManager sendWeibo:[RRTManager manager].loginManager.loginInfo.userId
                ...
                [self showImage:[UIImage imageNamed:@"confirm-err72.png"] status:@"发送微博失败"];
                isEndPublish = YES;
            }];
        }
    }
