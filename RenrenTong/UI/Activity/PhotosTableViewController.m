...
@property (nonatomic, assign) int recPageIndex;
@property (nonatomic, assign) int recPageSize;
...
-(void)Selectbutton:(UISegmentedControl *)Segment
{
    ...
    if (index == 0) {
        ...
        [self.netWorkManager recentlyAblum:[RRTManager manager].loginManager.loginInfo.tokenId
                                 pageIndex:self.recPageIndex
                                  pageSize:self.recPageSize
                                  TopCount:4
                                   success:^(NSArray *photoListArray) {
...
- (void)headerReresh
{
    ...
    if (index == 0) {
        ...
        [self.netWorkManager photoList:[RRTManager manager].loginManager.loginInfo.tokenId
            ...
            [self showImage:[UIImage imageNamed:@"confirm-err72.png"] status:@"刷新失败..."];
        ...
        self.recPageIndex = 1;
        [self.netWorkManager recentlyAblum:[RRTManager manager].loginManager.loginInfo.tokenId
                                 pageIndex:_self.recPageIndex
                                  pageSize:_self.recPageSize
                                  TopCount:4
                                   success:^(NSArray *photoListArray) {
                                        [self dismiss];
                                        _self.recPageIndex = 1;
