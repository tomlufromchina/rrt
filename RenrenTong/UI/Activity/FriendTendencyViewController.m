...
@interface FriendTendencyViewController ()<UIActionSheetDelegate,
                                            ...
                                            InputViewDelegate,
                                            UITextFieldDelegate,
                                            FriendTendencyCellDelegate,DetectionViewDelegate>
{
    NSString *_headerURL;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        [cell.userface setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://images.%@/avatars/%d.jpg",aedudomain, fr.userId]]];
    ...
    [self.netWorkManager getUIData:[RRTManager manager].loginManager.loginInfo.tokenId
                           success:^(NSMutableArray *data) {
                               if (data) {
                                   ...
                                   ActivityHeaderCell *cell = (ActivityHeaderCell*)[self.tableView cellForRowAtIndexPath:cellPath];
                                     ...
                                     pageSize:self.pageSize
                                      success:^(NSMutableArray *friendDynamic) {
                                          ...
                                          if ([friendDynamic count] == 0) {
                                              [self showImage:[UIImage imageNamed:@"confirm-err72.png"] status:@"没有更多的好友动态了哦！"];
