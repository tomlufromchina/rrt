...
- (void)updateUI:(NSArray *)data
{
    if ([data count] > 0) {
        for (int i = 0; i < [data count]; i ++) {
            ...
            self.imageView.image = [UIImage imageNamed:@"favour18x14.png"];
            ...
            if ([self.blogCommnts.Comments count] > 0 || self.blogCommnts.Total > 0) {
                ...
                for (int j = 0; j < [self.blogCommnts.Comments count]; j ++) {
                    ...
                    if (bc.AtUserId > 0) {
                        comments = [NSString stringWithFormat:@"%@%@%@", comments,[NSString stringWithFormat:@"U[%@]U%dI%dIC%dC  回复 U[%@]U%dI%dIC%dC :%@", bc.Author,bc.OwnerId,j,0,bc.AtNickName,bc.AtUserId,j,0,[self flattenHTML:bc.Body]],br];
                    }else{
                        comments = [NSString stringWithFormat:@"%@%@%@", comments,[NSString stringWithFormat:@"U[%@]U%dI%dIC%dC :%@", bc.Author,bc.OwnerId,j,0,[self flattenHTML:bc.Body]],br];
                    }
                }
                ...
                //scrollView contentSize:
                CGSize newSize = CGSizeMake(self.view.frame.size.width, self.chatBtn.bottom + 10);
                [self.scrollView setContentSize:newSize];
            }
...
- (void)mlEmojiLabel:(MLEmojiLabel*)emojiLabel didSelectLink:(NSString*)link withType:(MLEmojiLabelLinkType)type
{
    switch(type){
        case MLEmojiLabelLinkTypeUser:
        {
            ...
            self.commentIndex = [MLEmojiLabel getIndex:link];
        }
            break;
