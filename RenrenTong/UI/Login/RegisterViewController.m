...
- (void)registerClicked:(UIButton*)button
{
    ...
    if ([self validateTheLogin]) {
        if (![self CheckInput:self.accountTextField.text]) {
...
- (void)goBack
{
    [self showSuccessWithStatus:@"已注册成功，请到邮箱进行激活"];
...
- (BOOL)validateTheLogin
{
    if (self.identityTextField.text.length <= 0) {
        [self showImage:[UIImage imageNamed:@"confirm-err72.png"] status:@"请选择身份哦！"];

        return NO;
    }
    
    if (self.accountTextField.text.length <= 0) {
        [self showImage:[UIImage imageNamed:@"confirm-err72.png"] status:@"请输入您的邮箱哦！"];

        return NO;
    }
    
    if (self.nameTextField.text.length <= 0) {
        [self showImage:[UIImage imageNamed:@"confirm-err72.png"] status:@"请填写真实姓名哦！"];
