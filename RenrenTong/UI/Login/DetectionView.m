...
{
    //发微博按钮
    [self creatComposeButton:@"发微博" imageName:@"wb--" type:IWComposeButtonTypeSendWeiBo];
    
    // 传照片
    [self creatComposeButton:@"传照片" imageName:@"czp-" type:IWComposeButtonTypeSendPhotos];
    
    //写日志按钮
    [self creatComposeButton:@"写日志" imageName:@"rz-" type:IWComposeButtonTypeSendRecod];
...
- (void)setupOtherButtonsFrame
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        // 45 是自己添加上去的
        CGFloat btnX =  btnMargin + col * (btnWidth + btnMargin) - 10;
