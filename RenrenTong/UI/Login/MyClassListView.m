...
@implementation MyClassListView
...
{
    //文字按钮
    [self creatComposeButton:@"写文章" imageName:@"wb-" type:IWComposeButtonTypeSendTabulation];
    
    //相册按钮
    [self creatComposeButton:@"传照片" imageName:@"xc-" type:IWComposeButtonTypeSendAlbum];
...
- (void)setupOtherButtonsFrame
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        // 按钮的间隙
        CGFloat btnMargin = (self.width * 2.5 - (IWMacCols * btnWidth)) /(IWMacCols + 1);
        // 45 是自己添加上去的
        CGFloat btnX =  btnMargin + col * (btnWidth + btnMargin);
        CGFloat btnY = self.height + row * (btnHeight + btnMargin) + i * IWDatel + 80;
