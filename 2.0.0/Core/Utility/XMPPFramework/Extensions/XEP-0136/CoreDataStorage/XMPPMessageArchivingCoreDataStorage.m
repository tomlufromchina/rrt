...
- (void)archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream
{
	...
	[self scheduleBlock:^{
		...
		else
		{
            ...
            //add by jeffrey.liu
            archivedMessage.bRead = [NSNumber numberWithBool:NO];
			...
			if ([messageBody length] > 0)
			{
                ...
                //add by Jeffrey liu
                //这里不需要赋值，因为contact就是前一个contact message
//                contact.bToped = [NSNumber numberWithBool:NO];
//                contact.topedTimestamp = [[NSDate alloc] init];
