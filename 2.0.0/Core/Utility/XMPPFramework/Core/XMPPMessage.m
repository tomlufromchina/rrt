...
- (NSString *)subject
{
    ...
    for (NSXMLElement *subjectElement in [self elementsForName:@"subject"])
    {
        NSArray *attributes = [subjectElement attributes];
        if (!attributes || [attributes count] == 0) {
            subject = [subjectElement stringValue];
            break;
        }
    }
...
- (NSString *)subjectForLanguage:(NSString *)language
{
    NSString *subjectForLanguage = nil;
    
    for (NSXMLElement *subjectElement in [self elementsForName:@"subject"])
    {
        NSString *lang = [[subjectElement attributeForName:@"lang"] stringValue];
        
        // Openfire strips off the xml prefix
        if (lang == nil)
        {
            lang = [[subjectElement attributeForName:@"lang"] stringValue];
        }

        if (lang) {
            if ([language isEqualToString:lang])
            {
                subjectForLanguage = [subjectElement stringValue];
                break;
            }
        }
    }

    return subjectForLanguage;
}
...
- (void)addSubject:(NSString *)subject withLanguage:(NSString *)language
{
    NSXMLElement *subjectElement = [NSXMLElement elementWithName:@"subject" stringValue:subject];
    
    if ([language length])
    {
        [subjectElement addAttributeWithName:@"lang" stringValue:language];
    }
