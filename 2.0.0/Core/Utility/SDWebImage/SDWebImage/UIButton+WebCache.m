...
// add by bwzhu 2013-6-24
@implementation UIButton (WebCacheExpend)

- (void)setImageWithUrlStr:(NSString *)urlStr forState:(UIControlState)state placholderImage:(UIImage *)placeholder;
{
    ...
    [self setImageWithURL:[NSURL URLWithString:imgUrl] forState:UIControlStateNormal placeholderImage:placeholder];
    CFRelease((__bridge CFTypeRef)(imgUrl));
}

- (void)setBgImageWithUrlStr:(NSString *)urlStr forState:(UIControlState)state placholderImage:(UIImage *)placeholder
{
    NSString *imgUrl = (__bridge NSString *)CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)urlStr, nil, nil, kCFStringEncodingUTF8);
    [self setBackgroundImageWithURL:[NSURL URLWithString:imgUrl] forState:UIControlStateNormal placeholderImage:placeholder];
    CFRelease((__bridge CFTypeRef)(imgUrl));
}
