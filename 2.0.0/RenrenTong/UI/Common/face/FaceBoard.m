...
- (id)init
{
    self = [super initWithFrame:CGRectMake(0, 0, 320, FaceView_Height + PageControl_Height)];
    if (self) {
        ...
        //表情盘
        faceView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, 320, FaceView_Height)];
        faceView.pagingEnabled = YES;
        faceView.contentSize = CGSizeMake((FACE_TOTAL_NUM / FACE_PAGE_NUM + 1) * 320, FaceView_Height);
        ...
        for (int i = 0; i < FACE_TOTAL_NUM; i++) {
            ...
            (((i % FACE_PAGE_NUM) % 7) * (FACE_WIDTH + Interval_H) + Interval_H + (i / FACE_PAGE_NUM * 320),
