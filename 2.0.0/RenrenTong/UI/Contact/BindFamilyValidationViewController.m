...
- (void) sendVerification
{
    if (self.textView.text.length <= 0) {
        ...
        [self.netWorkManager FamilySendValidation:[RRTManager manager].loginManager.loginInfo.tokenId ParUserAccount:self.Account ValidTxt:self.textView.text success:^(NSDictionary *data) {
            [self dismiss];
            [self updateUI];
        } failed:^(NSString *errorMSG) {
            [self showErrorWithStatus:@"你已经绑定了该账号或已经绑定其他账号了哦!"];
        }];
